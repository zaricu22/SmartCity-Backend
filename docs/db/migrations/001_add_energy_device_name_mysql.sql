...
--
-- Run against smartcity schema. Safe to run once; re-running the ADD COLUMN
-- step will fail with "Duplicate column name" if already applied.
...
-- VARCHAR, not TEXT: MySQL rejects a DEFAULT on BLOB/TEXT/GEOMETRY/JSON columns,
-- and step 1 needs that temporary default to satisfy NOT NULL on existing rows.
ALTER TABLE smartcity.energy_device
    ADD COLUMN name VARCHAR(255) NOT NULL DEFAULT '' AFTER building_id;

-- ============================================================
-- 2. Backfill known rows (matches docs/db/seed-data.sql device names)
-- ============================================================

UPDATE smartcity.energy_device SET name = CASE id
...
UPDATE smartcity.energy_device
    SET name = CONCAT(type, ' ', SUBSTRING(id, 1, 8))
    WHERE name = '';
